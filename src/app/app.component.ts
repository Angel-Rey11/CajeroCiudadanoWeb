...
export class AppComponent implements OnInit, OnDestroy {
  ...
  public async startCall(): Promise<void> {
    ...
    try{
      this.call = await this.callService.stablishCall(call);
      this.openQeue();
    } catch (error) {
  ...
  public async endCall() {
    try{
      await this.callService.closeMediaCall();
      this.openRate();
      this.highlightStars();
    } catch (error) {
      const endCallToast = document.getElementById('endCallToast');
 ...
 */
  ...
  public async sendRating(rate:number){
    var Call: Call
    await this.callBDService.getCall(this.call.id).subscribe(data=>{
      Call = data;
      Call.rating = rate;
      this.callBDService.UpdateRating(Call.id,Call,Call.userId);
      console.log(Call);
    });
  ...
  /**
   * Metodo para recargar la página por si el cajero no esta disponible, se recargar la página para intentar
   * encontrar la IP de ese cajero
   */
  public reloadPage() {
